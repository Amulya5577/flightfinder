...
mongoose.connect('mongodb://127.0.0.1:27017/FlightBookingMERN', { 
    ...
    app.post('/login', async (req, res) => {
        ...
        try {
            ...
            if (usertype === 'admin') {
                user = await User.findOne({ email, usertype: 'admin', approval: 'approved' });
            } else if (usertype === 'customer') {
                user = await User.findOne({ email, usertype: 'customer', approval: 'approved' });
            } else if (usertype === 'flight-operator') {
                user = await User.findOne({ email, usertype: 'flight-operator', approval: 'approved' });
            } else {
                return res.status(400).json({ message: 'Invalid user type' });
            }

            if (!user) {
                return res.status(401).json({ message: 'Invalid email, user type, or not approved' });
            }
